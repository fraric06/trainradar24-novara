...
const SOURCES = {
  lombardia: [
    'https://www.dati.lombardia.it/api/views/3z4k-mxz9/rows.csv?accessType=DOWNLOAD',
...
function classify(route, trip){
  const s=`${trip.train_category||''} ${trip.trip_short_name||''} ${route.route_short_name||''} ${route.route_long_name||''}`.toUpperCase();
  if(/REGIONALE VELOCE|\bRV\b/.test(s)) return 'RV';
  if(/MALPENSA|MXP/.test(s)) return 'MXP';
  if(/SFM/.test(s)) return 'SFM';
  if(/^S\d/.test(route.route_short_name||'')) return 'SUB';
  return 'REG';
}
function normalize(zipBuffer, region){
  ...
  for(const t of trips){
    ...
    const category=classify(rt,t); const short=String(t.trip_short_name||'').trim();
...
async function load(){
  ...
  for(const [region,urls] of Object.entries(SOURCES)){
    ...
    for(const url of urls){
      try { const buf=await download(url); if(!buf || buf.length<1000) throw new Error('feed vuoto'); const trips=normalize(buf,region); if(!trips.length) throw new Error('nessuna corsa valida'); all.push(...trips); status[region]={ok:true,trips:trips.length,updatedAt:new Date().toISOString(),source:url}; ok=true; break; }
      catch(e){ last=e.message; }
    }
    if(!ok) status[region]={ok:false,error:last};
  }
