...
function minuteFromHHMM(v){if(typeof v!=="string"||!/^(\d{1,2}):(\d{2})/.test(v))return null;const m=v.match(/^(\d{1,2}):(\d{2})/);return Number(m[1])*60+Number(m[2]);}
...
// GTFS trip_id is an internal identifier. Real railway numbers are mapped by route and scheduled departure.
function scheduledTrainNumber(trip){
  ...
  if(trip.route!=="R27"||!/^1904\d+$/.test(raw))return null;
  const dep=trip.stops[0]?.dep_min;
  const r27={
    371:"4214",431:"4222",491:"4226",551:"4232",611:"4236",671:"4240",731:"4244",791:"4246",
    851:"4250",911:"4254",971:"4260",1031:"4266",1091:"4270",1151:"4276",1211:"4280"
  };
  return r27[dep]||null;
}
...
function vtDate(){return new Date().toLocaleString("en-US",{timeZone:"Europe/Rome",weekday:"short",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false,timeZoneName:"longOffset"}).replace(/,/g,"");}
function parseVT(item,stationId){const scheduled=Number(item?.orarioPartenza||0);const scheduledMin=scheduled?new Date(scheduled).toLocaleTimeString("it-IT",{timeZone:"Europe/Rome",hour:"2-digit",minute:"2-digit",hour12:false}):item?.compOrarioPartenza;const min=minuteFromHHMM(scheduledMin);if(min===null)return null;const delay=Number.isFinite(Number(item?.ritardo))?Math.max(0,Math.round(Number(item.ritardo))):0;return{stationId,number:String(item?.numeroTreno??""),scheduledMin:min,delayMin:delay,destination:item?.destinazione||null,cancelled:Number(item?.provvedimento)===1||item?.riprogrammazione==="Y"&&Number(item?.provvedimento)>0,updatedAt:new Date().toISOString()};}
async function refreshRealtime(){const records=[];let errors=[];const date=encodeURIComponent(vtDate());for(const stationId of STATION_IDS){try{const data=await fetchJson(`https://www.viaggiatreno.it/infomobilita/resteasy/viaggiatreno/partenze/${stationId}/${date}`);if(Array.isArray(data))for(const item of data){const x=parseVT(item,stationId);if(x)records.push(x)}}catch(e){errors.push(`${stationId}: ${e.message}`)}}if(records.length){realtime={available:true,updatedAt:new Date().toISOString(),records,error:errors.length?errors.join(" | "):null};console.log(`ViaggiaTreno: ${records.length} partenze realtime`)}else{realtime={...realtime,available:false,error:errors.join(" | ")||"Nessun dato realtime"};console.log(`ViaggiaTreno non disponibile: ${realtime.error}`)}}
function findRealtime(trip){let best=null;for(const stop of trip.stops){if(!STATION_IDS.includes(stop.id))continue;for(const r of realtime.records){if(r.stationId!==stop.id)continue;const diff=Math.abs(r.scheduledMin-stop.dep_min);if(diff>3)continue;if(!best||diff<best.diff)best={...r,diff}}}return best;}
app.get("/api/trains",(req,res)=>{try{const now=romeNow(),trains=[];for(const trip of PREPARED_TRIPS){if(!tripValidToday(trip))continue;const live=findRealtime(trip),delay=live?live.delayMin:0,pos=estimatePosition(trip,now,delay);if(!pos)continue;const scheduledNumber=scheduledTrainNumber(trip);trains.push({trip_id:trip.trip_id,route:trip.route,route_name:ROUTE_NAMES[trip.route]||trip.route,dep:trip.dep,arr:trip.arr,origin:trip.stops[0].name,destination:trip.stops.at(-1).name,train_number:live?.number||scheduledNumber||"",delay_min:delay,delay_known:Boolean(live),delay_status:delay<5?"on_time":delay<=30?"delayed":"severe_delay",realtime:Boolean(live),realtime_train_number:live?.number||scheduledNumber||"",realtime_updated_at:realtime.updatedAt,cancelled:live?.cancelled||false,...pos});}res.json({generated_at:new Date().toISOString(),reference_minutes:Math.round(now),source:realtime.available?"GTFS + ViaggiaTreno":"GTFS static",realtime_available:realtime.available,realtime_updated_at:realtime.updatedAt,count:trains.length,trains});}catch(e){console.error("/api/trains",e);res.status(500).json({error:e.message,trains:[]})}});
