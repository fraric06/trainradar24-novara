...
function minuteFromDate(v){if(!v)return null;const s=new Date(Number(v)).toLocaleTimeString("it-IT",{timeZone:"Europe/Rome",hour:"2-digit",minute:"2-digit",hour12:false});const m=s.match(/^(\d{1,2}):(\d{2})/);return m?Number(m[1])*60+Number(m[2]):null;}
function parseVT(item,stationId){const min=minuteFromDate(item?.orarioPartenza)||null;if(min===null)return null;const delay=Number.isFinite(Number(item?.ritardo))?Math.max(0,Math.round(Number(item.ritardo))):0;return{stationId,number:String(item?.numeroTreno??""),scheduledMin:min,delayMin:delay,destination:item?.destinazione||null,cancelled:Number(item?.provvedimento)===1,updatedAt:new Date().toISOString()};}
async function refreshRealtime(){const records=[];let errors=[];const date=encodeURIComponent(vtDate());for(const stationId of STATION_IDS){try{const data=await fetchJson(`https://www.viaggiatreno.it/infomobilita/resteasy/viaggiatreno/partenze/${stationId}/${date}`);if(Array.isArray(data))for(const item of data){const x=parseVT(item,stationId);if(x)records.push(x)}}catch(e){errors.push(`${stationId}: ${e.message}`)}}if(records.length)realtime={available:true,updatedAt:new Date().toISOString(),records,error:errors.length?errors.join(" | "):null};else realtime={...realtime,available:false,error:errors.join(" | ")||"Nessun dato realtime"};}
