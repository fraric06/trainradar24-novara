...
function scheduledTrainNumber(trip){const raw=String(trip.trip_id||"").split("-")[0];if(trip.route==="S6"&&/^124\d+$/.test(raw))return raw.slice(1);if(trip.route==="R25"&&/^111\d+$/.test(raw))return raw.slice(1);if(trip.route!=="R27")return null;const dep=trip.stops[0]?.dep_min;const r27={371:"4214",431:"4222",491:"4226",551:"4232",611:"4236",671:"4240",731:"4244",791:"4246",851:"4250",911:"4254",971:"4260",1031:"4266",1091:"4270",1151:"4276",1211:"4280"};return r27[dep]||null;}
let realtime={available:false,updatedAt:null,records:[],error:null};
function fetchJson(url){return new Promise((resolve,reject)=>{const req=https.get(url,{headers:{"User-Agent":"TrainRadar24-Novara/1.0","Accept":"application/json"}},res=>{let body="";res.setEncoding("utf8");res.on("data",c=>body+=c);res.on("end",()=>{if(res.statusCode===204)return reject(new Error("HTTP 204 No Content"));if(res.statusCode<200||res.statusCode>=300)return reject(new Error(`HTTP ${res.statusCode}`));try{resolve(JSON.parse(body))}catch(e){reject(new Error("JSON non valido"))}})});req.setTimeout(7000,()=>req.destroy(new Error("timeout")));req.on("error",reject);});}
function vtDate(){return new Date().toString();}
function minuteFromDate(v){if(v===null||v===undefined)return null;const n=Number(v);if(Number.isFinite(n)){const d=new Date(n);return d.getHours()*60+d.getMinutes()}const m=String(v).match(/(\d{1,2}):(\d{2})/);return m?Number(m[1])*60+Number(m[2]):null;}
...
function midnightMillis(){const d=new Date();d.setHours(0,0,0,0);return d.getTime();}
function parseAndamento(data,trip,number){if(!data||typeof data!=="object")return null;let delay=null;for(const key of ["ritardo","ritardoPartenza","ritardoArrivo"]){const n=Number(data[key]);if(Number.isFinite(n)){delay=Math.max(0,Math.round(n));break;}}
  const stops=Array.isArray(data.fermate)?data.fermate:[];const now=Date.now();let latest=null;
  for(const s of stops){const scheduled=Number(s.programmata??s.partenzaTeorica??s.arrivoTeorico);if(!Number.isFinite(scheduled))continue;if(scheduled<=now){const d=Number(s.ritardo);if(Number.isFinite(d))latest=Math.max(0,Math.round(d));}}
  if(delay===null&&latest!==null)delay=latest;if(delay===null)return null;
  return{stationId:trip.stops[0].id,number:String(number),scheduledMin:trip.stops[0].dep_min,delayMin:delay,cancelled:["ST","SI","SF"].includes(String(data.tipoTreno||"")),updatedAt:new Date().toISOString(),source:"andamentoTreno"};
}
async function refreshRealtime(){
  ...
  // The departure board stops showing trains after they leave. For trains currently in motion,
  // ask ViaggiaTreno's dedicated andamentoTreno endpoint using the commercial train number.
  const now=romeNow();
  const active=PREPARED_TRIPS.filter(t=>tripValidToday(t)&&now>=t.stops[0].dep_min-5&&now<=t.stops.at(-1).arr_min+5);
  for(const trip of active){const number=scheduledTrainNumber(trip);if(!number)continue;try{const data=await fetchJson(`https://www.viaggiatreno.it/infomobilita/resteasy/viaggiatreno/andamentoTreno/${trip.stops[0].id}/${number}/${midnightMillis()}`);const x=parseAndamento(data,trip,number);if(x)records.push(x)}catch(e){errors.push(`${number}: ${e.message}`)}}
  if(records.length){realtime={available:true,updatedAt:new Date().toISOString(),records,error:errors.length?errors.join(" | "):null};console.log(`ViaggiaTreno OK: ${records.length} realtime records`)}else{realtime={...realtime,available:false,error:errors.join(" | ")||"Nessun dato realtime"};console.log(`ViaggiaTreno NON DISPONIBILE: ${realtime.error}`)}
}
function findRealtime(trip,scheduledNumber){const candidates=realtime.records;if(scheduledNumber){const exact=candidates.find(r=>r.number===scheduledNumber&&r.stationId===trip.stops[0]?.id);if(exact)return exact;const anywhere=candidates.find(r=>r.number===scheduledNumber);if(anywhere)return anywhere;}let best=null;for(const r of candidates){if(r.stationId!==trip.stops[0]?.id)continue;const diff=Math.abs(r.scheduledMin-trip.stops[0].dep_min);if(diff<=3&&(!best||diff<best.diff))best={...r,diff};}return best;}
